...
def train(gen, dis, opt_gen, opt_dis, epoch, train_loader):
    ...
    for batch_idx, (I_l, I_r, I_m) in enumerate(train_loader):
        ...
        I_l, I_r, I_m = Variable(I_l).cuda(), Variable(I_r).cuda(), Variable(I_m).cuda()
        ...
        weight = gaussian_weight(batchSize, imgSize)
        ...
        mrf_loss = mrf((I_pred_split[1].cuda()+1)/2.0, (I_m.cuda()+1)/2.0) * 0.01
        ...
        real_label = torch.ones(batchSize,1).cuda()
        fake_label = torch.zeros(batchSize,1).cuda()
        ...
        gen_loss = pixel_rec_loss + mrf_loss.cuda() + feat_rec_loss + feat_cons_loss + gen_adv_loss
...
def test(gen, dis, epoch, test_loader):
